...
1. Read `run.json` and the artifacts named there before resuming a run. Never rely on conversation memory as state.
...
10. Never edit `creator/lessons.md` from lesson candidates without explicit, item-level human approval. Preserve all previously approved lessons unless their removal is separately approved.
...
- Keep secrets and confidential details out of Git. Record a sanitized confidentiality warning instead.
