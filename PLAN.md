...
- Git tracks the public framework and fictional examples. Active creator context and real runs live in a selected private data root that may optionally use its own private Git repository.
...
- [x] Add blank tracked creator templates without inventing preferences.
- [x] Implement private-root `init`, `data-root`, `new-run`, `status`, `validate`, and `count`.
